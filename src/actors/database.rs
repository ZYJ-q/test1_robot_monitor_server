...
pub fn get_one_traders(pool: web::Data<Pool>, tra_id: &str) -> Result<HashMap<String, Trader>> {
    ...
    let res = conn
    .exec_first(
                r"select * from test1_traders where tra_id = :tra_id",
                params! {
                        "tra_id" => tra_id
                        },
                )
...
// 获取回测权益xh01数据
pub fn get_bian_xh01_equity1(
    pool: web::Data<Pool>,
) -> Result<Vec<BianEquity>> {
    let mut conn = pool.get_conn().unwrap();
    // let mut re: Vec<Trade> = Vec::new();
        let equitys = conn.query_map(
            "select * from  test_two_bian_xh01_equity order by time",
            |(id, name, time, equity, r#type)| {
                BianEquity{id, name, time, equity, r#type}
            }
            ).unwrap();
...
// 获取回测权益xh01数据
pub fn get_bian_xh01_equity2(
    pool: web::Data<Pool>,
) -> Result<Vec<BianEquity>> {
    let mut conn = pool.get_conn().unwrap();
    // let mut re: Vec<Trade> = Vec::new();
        let equitys = conn.query_map(
            "select * from  test_three_bian_xh01_equity order by time",
            |(id, name, time, equity, r#type)| {
                BianEquity{id, name, time, equity, r#type}
            }
            ).unwrap();
...
// 获取回测权益xh01数据
pub fn get_bian_xh01_equity3(
    pool: web::Data<Pool>,
) -> Result<Vec<BianEquity>> {
    let mut conn = pool.get_conn().unwrap();
    // let mut re: Vec<Trade> = Vec::new();
        let equitys = conn.query_map(
            "select * from  test_four_bian_xh01_equity order by time",
            |(id, name, time, equity, r#type)| {
                BianEquity{id, name, time, equity, r#type}
            }
            ).unwrap();
...
// 获取回测权益xh01数据
pub fn get_bian_xh01_equity4(
    pool: web::Data<Pool>,
) -> Result<Vec<BianEquity>> {
    let mut conn = pool.get_conn().unwrap();
    // let mut re: Vec<Trade> = Vec::new();
        let equitys = conn.query_map(
            "select * from  test_five_bian_xh01_equity order by time",
            |(id, name, time, equity, r#type)| {
                BianEquity{id, name, time, equity, r#type}
            }
            ).unwrap();
...
// 获取回测权益xh01数据
pub fn get_bian_xh01_equity5(
    pool: web::Data<Pool>,
) -> Result<Vec<BianEquity>> {
    let mut conn = pool.get_conn().unwrap();
    // let mut re: Vec<Trade> = Vec::new();
        let equitys = conn.query_map(
            "select * from  test_six_bian_xh01_equity order by time",
            |(id, name, time, equity, r#type)| {
                BianEquity{id, name, time, equity, r#type}
            }
            ).unwrap();
...
pub fn update_positions(pool: web::Data<Pool>, name:&str, threshold:&str) -> Result<()> {
    let mut conn = pool.get_conn().unwrap();
    let res = conn.exec_drop(
        r"update test1_traders set threshold = :threshold where name = :name",
        params! {
            "name" => name,
            "threshold" => threshold
        },
...
pub fn update_ori_balance(pool: web::Data<Pool>, tra_id:&str, ori_balance:&str) -> Result<()> {
    let mut conn = pool.get_conn().unwrap();
    let res = conn.exec_drop(
        r"update test1_traders set ori_balance = :ori_balance where tra_id = :tra_id",
        params! {
            "tra_id" => tra_id,
...
pub fn update_alarms(pool: web::Data<Pool>, name:&str, alarm:&str) -> Result<()> {
    let mut conn = pool.get_conn().unwrap();
    let res = conn.exec_drop(
        r"update test1_traders set alarm = :alarm where name = :name",
        params! {
            "name" => name,
            "alarm" => alarm
        },
...
pub fn delect_accounts(pool: web::Data<Pool>, tra_id:&str) -> Result<()> {
    let mut conn = pool.get_conn().unwrap();
    let res = conn.exec_drop(
        r"delete from test1_traders where tra_id = :tra_id",
        params! {
            "tra_id" => tra_id
        },
    );
...
pub fn add_accounts(pool: web::Data<Pool>, name:&str, api_key: &str, secret_key:&str, alarm:&str, threshold:&str) -> Result<()> {
    let mut conn = pool.get_conn().unwrap();
    let res = conn.exec_drop(
        r"INSERT INTO test1_traders (tra_venue, ori_balance, tra_currency, api_key, secret_key, other_keys, type, name, alarm, threshold)
...
pub fn select_id(pool: web::Data<Pool>, name: &str, prod_id: &str) -> Result<()> {
    ...
    let res:Result<Vec<u64>> = conn.exec(
        "select tra_id from test1_traders where name = :name", 
        params! {
            "name" => name
        },
    );
