...
// 找到用户只能看那个账户权限
pub async fn select_traders(mut payload: web::Payload, db_pool: web::Data<Pool>) -> Result<HttpResponse, Error> {
    // payload is a stream of Bytes objects
    let mut body = web::BytesMut::new();
    while let Some(chunk) = payload.next().await {
        let chunk = chunk?;
...
// 获取xh01权益数据（显示资金曲线）
pub async fn get_bian_xh01_equity(mut payload: web::Payload, db_pool: web::Data<Pool>) -> Result<HttpResponse, Error> {
    // payload is a stream of Bytes objects
    let mut body = web::BytesMut::new();
    while let Some(chunk) = payload.next().await {
        let chunk = chunk?;
    ...
    let date =  database::get_bian_xh01_equity(db_pool.clone());
        match date {
            Ok(traders) => {
                return Ok(HttpResponse::Ok().json(Response {
                    status: 200,
...
// 获取xh01权益数据（显示资金曲线）
pub async fn get_bian_xh01_equity_one(mut payload: web::Payload, db_pool: web::Data<Pool>) -> Result<HttpResponse, Error> {
    // payload is a stream of Bytes objects
    let mut body = web::BytesMut::new();
    while let Some(chunk) = payload.next().await {
        let chunk = chunk?;
    ...
    let date =  database::get_bian_xh01_equity1(db_pool.clone());
        match date {
            Ok(traders) => {
                return Ok(HttpResponse::Ok().json(Response {
                    status: 200,
...
// 获取xh01权益数据（显示资金曲线）
pub async fn get_bian_xh01_equity_two(mut payload: web::Payload, db_pool: web::Data<Pool>) -> Result<HttpResponse, Error> {
    // payload is a stream of Bytes objects
    let mut body = web::BytesMut::new();
    while let Some(chunk) = payload.next().await {
        let chunk = chunk?;
    ...
    let date =  database::get_bian_xh01_equity2(db_pool.clone());
        match date {
            Ok(traders) => {
                return Ok(HttpResponse::Ok().json(Response {
                    status: 200,
...
// 获取xh01权益数据（显示资金曲线）
pub async fn get_bian_xh01_equity_three(mut payload: web::Payload, db_pool: web::Data<Pool>) -> Result<HttpResponse, Error> {
    // payload is a stream of Bytes objects
    let mut body = web::BytesMut::new();
    while let Some(chunk) = payload.next().await {
        let chunk = chunk?;
    ...
    let date =  database::get_bian_xh01_equity3(db_pool.clone());
        match date {
            Ok(traders) => {
                return Ok(HttpResponse::Ok().json(Response {
                    status: 200,
...
// 获取xh01权益数据（显示资金曲线）
pub async fn get_bian_xh01_equity_four(mut payload: web::Payload, db_pool: web::Data<Pool>) -> Result<HttpResponse, Error> {
    // payload is a stream of Bytes objects
    let mut body = web::BytesMut::new();
    while let Some(chunk) = payload.next().await {
        let chunk = chunk?;
    ...
    let date =  database::get_bian_xh01_equity4(db_pool.clone());
        match date {
            Ok(traders) => {
                return Ok(HttpResponse::Ok().json(Response {
                    status: 200,
...
// 获取xh01权益数据（显示资金曲线）
pub async fn get_bian_xh01_equity_five(mut payload: web::Payload, db_pool: web::Data<Pool>) -> Result<HttpResponse, Error> {
    // payload is a stream of Bytes objects
    let mut body = web::BytesMut::new();
    while let Some(chunk) = payload.next().await {
        let chunk = chunk?;
    ...
    let date =  database::get_bian_xh01_equity5(db_pool.clone());
        match date {
            Ok(traders) => {
                return Ok(HttpResponse::Ok().json(Response {
                    status: 200,
...
// 获取trader02权益数据（显示资金曲线）
pub async fn get_bian_trader02_equity_one(mut payload: web::Payload, db_pool: web::Data<Pool>) -> Result<HttpResponse, Error> {
    // payload is a stream of Bytes objects
    let mut body = web::BytesMut::new();
    while let Some(chunk) = payload.next().await {
        let chunk = chunk?;
    ...
    let date =  database::get_bian_trader02_equity(db_pool.clone());
        match date {
            Ok(traders) => {
                return Ok(HttpResponse::Ok().json(Response {
                    status: 200,
...
// 获取trader02权益数据（显示资金曲线）
pub async fn get_bian_trader02_equity_two(mut payload: web::Payload, db_pool: web::Data<Pool>) -> Result<HttpResponse, Error> {
    // payload is a stream of Bytes objects
    let mut body = web::BytesMut::new();
    while let Some(chunk) = payload.next().await {
        let chunk = chunk?;
    ...
    let date =  database::get_bian_trader02_equity2(db_pool.clone());
        match date {
            Ok(traders) => {
                return Ok(HttpResponse::Ok().json(Response {
                    status: 200,
...
// 获取trader02权益数据（显示资金曲线）
pub async fn get_bian_trader02_equity_three(mut payload: web::Payload, db_pool: web::Data<Pool>) -> Result<HttpResponse, Error> {
    // payload is a stream of Bytes objects
    let mut body = web::BytesMut::new();
    while let Some(chunk) = payload.next().await {
        let chunk = chunk?;
    ...
    let date =  database::get_bian_trader02_equity3(db_pool.clone());
        match date {
            Ok(traders) => {
                return Ok(HttpResponse::Ok().json(Response {
                    status: 200,
...
// 获取trader04权益数据（显示资金曲线）
pub async fn get_bian_trader04_equity_one(mut payload: web::Payload, db_pool: web::Data<Pool>) -> Result<HttpResponse, Error> {
    // payload is a stream of Bytes objects
    let mut body = web::BytesMut::new();
    while let Some(chunk) = payload.next().await {
        let chunk = chunk?;
    ...
    let date =  database::get_bian_trader04_equity(db_pool.clone());
        match date {
            Ok(traders) => {
                return Ok(HttpResponse::Ok().json(Response {
                    status: 200,
...
// 获取trader04权益数据（显示资金曲线）
pub async fn get_bian_trader04_equity_two(mut payload: web::Payload, db_pool: web::Data<Pool>) -> Result<HttpResponse, Error> {
    // payload is a stream of Bytes objects
    let mut body = web::BytesMut::new();
    while let Some(chunk) = payload.next().await {
        let chunk = chunk?;
    ...
    let date =  database::get_bian_trader04_equity2(db_pool.clone());
        match date {
            Ok(traders) => {
                return Ok(HttpResponse::Ok().json(Response {
                    status: 200,
...
// 获取trader04权益数据（显示资金曲线）
pub async fn get_bian_trader04_equity_three(mut payload: web::Payload, db_pool: web::Data<Pool>) -> Result<HttpResponse, Error> {
    // payload is a stream of Bytes objects
    let mut body = web::BytesMut::new();
    while let Some(chunk) = payload.next().await {
        let chunk = chunk?;
    ...
    let date =  database::get_bian_trader04_equity3(db_pool.clone());
        match date {
            Ok(traders) => {
                return Ok(HttpResponse::Ok().json(Response {
                    status: 200,
...
// 获取trader04权益数据（显示资金曲线）
pub async fn get_bian_trader04_equity_four(mut payload: web::Payload, db_pool: web::Data<Pool>) -> Result<HttpResponse, Error> {
    // payload is a stream of Bytes objects
    let mut body = web::BytesMut::new();
    while let Some(chunk) = payload.next().await {
        let chunk = chunk?;
    ...
    let date =  database::get_bian_trader04_equity4(db_pool.clone());
        match date {
            Ok(traders) => {
                return Ok(HttpResponse::Ok().json(Response {
                    status: 200,
...
// 获取xh02权益数据（显示资金曲线）
pub async fn get_bian_xh02_equity_one(mut payload: web::Payload, db_pool: web::Data<Pool>) -> Result<HttpResponse, Error> {
    // payload is a stream of Bytes objects
    let mut body = web::BytesMut::new();
    while let Some(chunk) = payload.next().await {
        let chunk = chunk?;
    ...
    let date =  database::get_bian_xh02_equity(db_pool.clone());
        match date {
            Ok(traders) => {
                return Ok(HttpResponse::Ok().json(Response {
                    status: 200,
...
// 获取xh02权益数据（显示资金曲线）
pub async fn get_bian_xh02_equity_two(mut payload: web::Payload, db_pool: web::Data<Pool>) -> Result<HttpResponse, Error> {
    // payload is a stream of Bytes objects
    let mut body = web::BytesMut::new();
    while let Some(chunk) = payload.next().await {
        let chunk = chunk?;
    ...
    let date =  database::get_bian_xh02_equity2(db_pool.clone());
        match date {
            Ok(traders) => {
                return Ok(HttpResponse::Ok().json(Response {
                    status: 200,
...
// 获取xh02权益数据（显示资金曲线）
pub async fn get_bian_xh02_equity_three(mut payload: web::Payload, db_pool: web::Data<Pool>) -> Result<HttpResponse, Error> {
    // payload is a stream of Bytes objects
    let mut body = web::BytesMut::new();
    while let Some(chunk) = payload.next().await {
        let chunk = chunk?;
    ...
    let date =  database::get_bian_xh02_equity3(db_pool.clone());
        match date {
            Ok(traders) => {
                return Ok(HttpResponse::Ok().json(Response {
                    status: 200,
...
// 获取xh02权益数据（显示资金曲线）
pub async fn get_bian_xh02_equity_four(mut payload: web::Payload, db_pool: web::Data<Pool>) -> Result<HttpResponse, Error> {
    // payload is a stream of Bytes objects
    let mut body = web::BytesMut::new();
    while let Some(chunk) = payload.next().await {
        let chunk = chunk?;
    ...
    let date =  database::get_bian_xh02_equity4(db_pool.clone());
        match date {
            Ok(traders) => {
                return Ok(HttpResponse::Ok().json(Response {
                    status: 200,
